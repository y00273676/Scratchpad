...
- **Auto Save From Clipboard**：默认开启，自动将系统剪贴板最新一条文本同步到输入框；可在菜单 **Edit → Auto Save From Clipboard** 关闭
- 空内容时显示占位提示，获得焦点后隐藏
- 双击窗口顶部区域可最大化 / 还原
- 无持久化、无数据库、无账号体系
...
│   ├── ClipboardReader.swift  # 剪贴板读取
│   ├── AppStorageKey.swift    # 用户设置键名
...
- **Auto Save From Clipboard**: enabled by default; syncs the latest system clipboard text into the editor. Toggle via **Edit → Auto Save From Clipboard**
